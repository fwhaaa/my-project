...
import { Table, Button, Input, Modal, Form, Message, Radio } from '@arco-design/web-react';
const RadioGroup = Radio.Group;
import httpServer from '../../httpServer';

const FormItem = Form.Item;
function SaqList(props) {
  ...
  async function getList() {
    httpServer({
      url:`/question/questionList/saq?subject=${metaInfo.subject}`,
      method: 'GET'
    })
    .then((res) => {
      console.log('----res',res);
  ...
  async function addQuestion(props){
    form.validate().then(async () => {
      ...
      // }, 1500);
      if( questionList?.some((v)=> v.type === 'saq' && v.questionId ===  currentRecord.id)){
